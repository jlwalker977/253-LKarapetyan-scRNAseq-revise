...
```
```{r, "gene filters"}
filter_uncharacterized_genes <- function(some_gene_list) {
...
filter_uncharacterized_genes <- function(de) {        # ← moved here, no other changes
  ...
  filtered_list <- lapply(some_gene_list, function(df) {   # ← was deg_list, now some_gene_list
    genes <- rownames(df)
    keep  <- !grepl(combined_pattern, genes, perl = TRUE)
    df[keep, ]
  })
...
v=8
suppressWarnings({
    out.gene.lists <- file.path(results,sprintf('DEG_contrasts_PBMC_wgf_%s_v%d',test,v),'csv')
    dir.create(out.gene.lists,recursive = TRUE)
    filt.out.gene.lists <- file.path(results,sprintf('DEG_contrasts_PBMC_wgf_%s_v%d',test,v),'filtered_csv')
    dir.create(filt.out.gene.lists,recursive = TRUE)
    out.de.volcano <- file.path(results,sprintf('DEG_contrasts_PBMC_wgf_%s_v%d',test,v),'volcano')
    dir.create(out.de.volcano,recursive=TRUE)
    out.de.heatmap <- file.path(results,sprintf('DEG_contrasts_PBMC_wgf_%s_v%d',test,v),'heatmap')
...
for (tag in use.tags){
    ...
    if (tag == 'timepoint'){
      ...
      time.sub <- dat_pmde1                # keep all timepoints for comparison
    } else {
...
filter_uncharacterized_genes <- function(some_gene_list) {
  ...
  # ── Single dataframe (loop usage: de) ────────────────────────────────────
  if (is.data.frame(some_gene_list)) {
    # Genes in rownames
    if (!is.null(rownames(some_gene_list)) &&
        any(grepl("^[A-Z]", rownames(some_gene_list)))) {
      genes <- rownames(some_gene_list)
      keep  <- !grepl(combined_pattern, genes, perl = TRUE)
      return(some_gene_list[keep, ])
    }
    # Genes in Gene column
    if ("Gene" %in% colnames(some_gene_list)) {
      keep <- !grepl(combined_pattern, some_gene_list$Gene, perl = TRUE)
      return(some_gene_list[keep, ])
    }
  }
  
  # ── Named list of dataframes (bulk usage: deg_list) ───────────────────────
  filtered_list <- lapply(some_gene_list, function(df) {
    genes <- rownames(df)
    keep  <- !grepl(combined_pattern, genes, perl = TRUE)
    df[keep, ]
  })
