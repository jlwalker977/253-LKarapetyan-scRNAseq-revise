#PBMC DEG loop corrected 042926 v2
...
cat("PBMC setup for loop")
dat_p = qread("dat-pbmc-scdblfinder-doublets-removed-042726-light.qs")
...
cell.types <- unique(dat_p@meta.data$gen_cell_types2)
...
suppressWarnings({
  out.gene.lists <- file.path(results, sprintf('DEG_contrasts_PBMC_wbgf_%s_v%d', test, v), 'csv')
  dir.create(out.gene.lists, recursive = TRUE)
  filt.out.gene.lists <- file.path(results, sprintf('DEG_contrasts_PBMC_wbgf_%s_v%d', test, v), 'filtered_csv')
  dir.create(filt.out.gene.lists, recursive = TRUE)
  out.de.volcano <- file.path(results, sprintf('DEG_contrasts_PBMC_wbgf_%s_v%d', test, v), 'volcano')
  dir.create(out.de.volcano, recursive = TRUE)
  out.de.heatmap <- file.path(results, sprintf('DEG_contrasts_PBMC_wbgf_%s_v%d', test, v), 'heatmap')
