...
v=8
suppressWarnings({
    out.gene.lists <- file.path(results,sprintf('DEG_contrasts_TIL_wgf_%s_v%d',test,v),'csv')
    dir.create(out.gene.lists,recursive = TRUE)
    filt.out.gene.lists <- file.path(results,sprintf('DEG_contrasts_TIL_wgf_%s_v%d',test,v),'filtered_csv')
    dir.create(filt.out.gene.lists,recursive = TRUE)
    out.de.volcano <- file.path(results,sprintf('DEG_contrasts_TIL_wgf_%s_v%d',test,v),'volcano')
    dir.create(out.de.volcano,recursive=TRUE)
    out.de.heatmap <- file.path(results,sprintf('DEG_contrasts_TIL_wgf_%s_v%d',test,v),'heatmap')
...
```{r}
til_markers3 <- FindAllMarkers(dat_tc4_presdf, 
                               only.pos = TRUE,         # Only look for upregulated genes
                               min.pct = 0.25,          # Gene must be in at least 25% of cells
                               logfc.threshold = 0.25)  # Minimum logFC

top_genes3 <- til_markers3 %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
top_genes3
```
